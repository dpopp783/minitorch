...
def test_other(a: float, b: float, c: float) -> None:
    ...
    """
    assert_close(add(add(a, b), c), add(a, add(b, c)))  # associative property of addition
    assert_close(mul(mul(a, b), c), mul(a, mul(b, c)))  # associative property of multiplication
    assert_close(add(a, 0.0), id(a))  # identity property of addition
    assert_close(mul(a, 1.0), id(a))  # identity property of multiplication
    assert_close(add(a, neg(a)), 0.0)  # additive inverse
    # assert_close(mul(a, inv(a)), 1.0)  # multiplicative inverse
    # NOTE: multiplicative inverse property fails due to floating point precision near zero,
    # will be interest to come back to but not the priority rn
